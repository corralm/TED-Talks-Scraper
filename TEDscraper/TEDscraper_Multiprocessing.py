...
#     def create_topics_csv(self): # won't work
#         """Creates CSV of all topics available from TED."""
#         soup = self.make_soup(session, 'https://www.ted.com/topics')
#         topic_list = []
...
class URLs(SoupMaker):
    ...
    async def get_all_url_paths(self, session):
        ...
        max_page = await self.get_max_page(session)
        ...
        for i in page_range:
            ...
            for attempt in range(2):
                try:
                    ...
                    soup = await self.make_soup(session, talks_page_url)
                    # delay between searches
                    await self.sleep_short()
    ...
    async def get_all_urls(self, session):
        ...
        url_list = []
        for url in await self.get_all_url_paths(session):
    ...
    async def clean_urls(self, urls):
        """Returns list of clean urls from urls the user inputs."""
        clean_urls = []
        for idx, url in enumerate(urls):
            if await url.startswith('https://www.ted.com/talks'):
                parts = url.split('/')
                joined = '/'.join(parts[:5])
                clean = joined.split('?')
    ...
    async def remove_urls_with_issues(self, session):
        """Remove urls with known issues to prevent unnecessary scraping."""
        urls = await self.all_urls(session)
        final_urls = []
        removed_urls = []
        removed_counter = 0
        issues_df = pd.read_csv('../data/known_issues.csv')
    ...
    async def all_urls(self, session):
        """Return all urls based on parameter 'urls' without removing."""
        # define url attribute
        if self.urls == 'all':
            urls = await self.get_all_urls(session)
        else:
            if isinstance(self.urls, list):
                urls = await self.clean_urls(self.urls)
            else:
                raise ValueError("'urls' param needs to be a list")
        return urls

    async def final_urls(self, session):
        """Return final urls to fetch."""
        # define url attribute
        if self.force_fetch:
            urls = await self.all_urls(session)
        else:
            urls = await self.remove_urls_with_issues(session)  
        return urls
...
class TEDscraper(TalkFeatures, URLs):
    ...
    async def scrape_all_features(self, session, soup):
...
#         for url in urls:
        ...
        for i in range(1, 4):
            ...
            try:
                # make soup
                soup = await self.make_soup(session, url)                                        
                # create nested dict
                self.ted_dict[self.dict_id] = {}
                # scrape features and add to a nested dict
                await self.scrape_all_features(session, soup)
...
#         return self.ted_dict
    ...
    async def bulk_crawl(self):
        async with ClientSession() as session:
            print("Fetching urls...\n")
            urls = await self.final_urls(session)
            print(f"Scraping {len(urls)} TED talks in '{self.language}'...")
            print(f"Estimated time to complete is {round((.9*len(urls)/60), 1)} minutes\n")
            tasks = []
            for url in urls:
                tasks.append(self.get_data(session, url))
...
if __name__ == "__main__":
    urls = [
    'https://www.ted.com/talks/rachel_pike_the_science_behind_a_climate_headline/transcript?language=mg',
    'https://www.ted.com/talks/eduardo_paes_the_4_commandments_of_cities/transcript?language=mg',
    'https://www.ted.com/talks/ludwick_marishane_a_bath_without_water/transcript?language=mg',
    'https://www.ted.com/talks/candy_chang_before_i_die_i_want_to/transcript?language=mg',
    'https://www.ted.com/talks/adam_grosser_a_mobile_fridge_for_vaccines/transcript?language=mg',
    'https://www.ted.com/talks/richard_st_john_8_secrets_of_success/transcript?language=mg',
    ]


    # instantiate the scraper & pass in optional arguments
    scraper = TEDscraper(lang_code='eo')

    # run
    ted_dict = asyncio.run(scraper.bulk_crawl())
    
    # transform the dictionary to a sorted pandas DataFrame
    df = scraper.to_dataframe(ted_dict)
    
    # output DataFrame as CSV
    df.to_csv('../data/ted_talk_test.csv', index=False)
